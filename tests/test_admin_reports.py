...
def test_admin_endpoints_require_jwt(client):
    # no token -> 401
    resp = client.get('/admin/reports/attendance-frequency')
...
def test_admin_blocked_for_non_admin(client, create_user):
    ...
    resp = client.get('/admin/reports/attendance-frequency', headers=headers)
...
def test_reports_aggregations(client, create_user, create_member, db_session):
    ...
    resp = client.get('/admin/reports/attendance-frequency?days=7&top=5', headers=headers)
    ...
    # members-activity
    resp = client.get('/admin/reports/members-activity?days=7', headers=headers)
    assert resp.status_code == 200
    data = resp.get_json()
